...
from crfnet.utils.reporting import TrainingReportCallback
...
def main():
    ...
    # Load best model
    best_model = keras.models.load_model(os.path.join(cfg.save_model, cfg.model_name + '.h5'), custom_objects=backbone.custom_objects, compile=False)
    # load anchor parameters, or pass None (so that defaults will be used)
    if 'small' in cfg.anchor_params:
        anchor_params = AnchorParameters.small
    else:
