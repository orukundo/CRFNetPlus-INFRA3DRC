...
class Evaluate(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # Modern TensorBoard callbacks no longer expose `.writer`, and
        # tf.Summary / writer.add_summary are TensorFlow 1.x APIs.
        # Write evaluation scalars through tf.summary instead.
        if self.tensorboard is not None:
            import tensorflow as tf

            log_dir = getattr(self.tensorboard, "log_dir", None)

            if log_dir:
                writer = tf.summary.create_file_writer(log_dir)

                with writer.as_default():
                    tf.summary.scalar("mAP_val", self.mean_ap, step=epoch)
                    tf.summary.scalar(
                        "precision_val", self.mean_precision, step=epoch
                    )
                    tf.summary.scalar(
                        "recall_val", self.mean_recall, step=epoch
                    )

                    if self.distance:
                        tf.summary.scalar(
                            "mADE_val", self.mean_loss_error, step=epoch
                        )
                        tf.summary.scalar(
                            "mRDE_val", self.mean_loss_error_rel, step=epoch
                        )
                    ...
                    for label, (
                        ...
                        class_name = self.generator.label_to_name(label)

                        if class_name != "bg":
                            tf.summary.scalar(
                                "ap_val_{}_{}_instances".format(
                                    class_name,
                                    int(num_annotations),
                                ),
                                average_precision,
