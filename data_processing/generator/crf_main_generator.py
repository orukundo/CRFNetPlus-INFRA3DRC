...
def create_generators(cfg, backbone):
  ...
  if 'nuscenes' in cfg.data_set:
    ...
    import ast
    from crfnet.data_processing.generator.infra3drc_generator import Infra3DRCGenerator

    try:
      scene_numbers = ast.literal_eval(cfg.scene_selection)
    except Exception:
      scene_numbers = [1,2,3,4,5,6]

    infra_args = dict(common_args)
    infra_args['seed'] = cfg.seed

    train_generator = Infra3DRCGenerator(
      cfg.data_path,
    ...
    eval_args = dict(infra_args)
    eval_args['camera_dropout'] = 0
    eval_args['radar_dropout'] = 0

    validation_generator = Infra3DRCGenerator(
      cfg.data_path,
      scene_numbers=scene_numbers,
      split='val',
      category_mapping=category_mapping,
    ...
    test_generator = Infra3DRCGenerator(
      cfg.data_path,
      scene_numbers=scene_numbers,
      split='test',
      category_mapping=category_mapping,
