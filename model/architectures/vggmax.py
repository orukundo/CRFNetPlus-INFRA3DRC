...
def custom(*args, **kwargs):
    # Keras 2.11 compatibility:
    # keras_modules_injection was removed from keras.applications.
    # keras-applications still expects backend/layers/models/utils via kwargs.
    kwargs.setdefault('backend', keras.backend)
    kwargs.setdefault('layers', keras.layers)
    kwargs.setdefault('models', keras.models)
    kwargs.setdefault('utils', keras.utils)
...
def min_pool2d(x, padding='valid'):
    ...
    if K.int_shape(x)[2] is not None and K.int_shape(x)[2] <= 20:
        padding = 'same'
